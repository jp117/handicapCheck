    ...
    for row in values[1:]:
        if len(row) >= 6:
            ...
            try:
                ...
                continue
            post_percentage[str(name).strip()] = pct_val
...
for row in sheet.iter_rows(min_row=2):
    name = row[5].value  # Column F
    if name:
        golfer_names.append(str(name).strip())

# Get Google Sheet ID from env or prompt
...
doc_name = base_name.replace('Golfer Spreadsheet V1', '- Handicap Check')
if not doc_name.lower().endswith('.docx'):
    doc_name = os.path.splitext(doc_name)[0] + '.docx'
